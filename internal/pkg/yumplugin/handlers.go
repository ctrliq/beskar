...
func repomdHandler(registryAddr string) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		vars := mux.Vars(r)

		rawRef := filepath.Join(registryAddr, "yum", vars["repository"], "repodata:latest")
		...
		manifest, err := oras.GetManifest(ref)
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
		...
		for _, layer := range manifest.Layers {
			...
			uri := fmt.Sprintf(
				"http://%s/v2/yum/%s/repodata/blobs/%s",
				registryAddr, vars["repository"], layer.Digest.String(),
			)
